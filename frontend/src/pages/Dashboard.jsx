...
export default function Dashboard() {
  ...
  const handleCancel = async (id) => {
    if (!globalThis.confirm('¿Estás seguro de que deseas cancelar esta reserva?')) return;
    setCancelling(id);
    try {
  ...
  const emptyMessage = EMPTY_MESSAGES[filter] || EMPTY_MESSAGES.all;
  ...
  if (loading) {
    ...
    bookingContent = (
      <div className="bg-white rounded-2xl p-12 text-center shadow-sm">
        ...
        <Link to="/search?city=Bucaramanga" className="btn-primary">Buscar hoteles</Link>
      </div>
    );
  } else {
    bookingContent = (
      <div className="space-y-4">
        {filtered.map(booking => {
          const nights = Math.ceil((new Date(booking.check_out) - new Date(booking.check_in)) / MS_PER_DAY);
                ...
                <div className="flex-1 p-5 flex flex-col justify-between">
                  <div className="flex items-start justify-between gap-3 flex-wrap">
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">Reserva #{booking.id} · {formatDate(booking.created_at)}</p>
                    </div>
                  ...
                  <div className="mt-3 flex items-center justify-between flex-wrap gap-3">
                    <div className="flex items-center gap-4 text-sm text-gray-600">
                      ...
                      <span className="flex items-center gap-1">
                        ...
                        {booking.guests} {Number.parseInt(booking.guests, 10) === 1 ? 'huésped' : 'huéspedes'}
                      </span>
                    </div>
